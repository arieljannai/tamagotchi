...
namespace HelloWorld
{
    ...
    public partial class MainWindow : Window
    {
                ...
                if (hand != null)
                {
                    ...
                    if (handWaving)
                    {
                        this.Dispatcher.Invoke(System.Windows.Threading.DispatcherPriority.Normal, new Action(delegate()
                        {
                            ...
                            isHandOnPic = IsHandOverThePic(handData, new Thickness(Schlez.Margin.Left,
                                                                                   Schlez.Margin.Top,
                                                                                   Schlez.Margin.Right,
                                                                                   Schlez.Margin.Bottom));
                        }));
                    }
                }
        ...
        private bool IsHandOverThePic(PXCMHandData handOutput, Thickness picloc, long timeStamp = 0)
        {
            //Iterate hands
            ...
            return IsJointOverThePic(nodes, numOfHands, picloc);
        }

        /* Check If current frames hand joints is Over The Picture */
        public bool IsJointOverThePic(PXCMHandData.JointData[][] nodes, int numOfHands, Thickness picloc)
        {
            ...
            this.Dispatcher.Invoke(System.Windows.Threading.DispatcherPriority.Normal, new Action(delegate()
            {
                Console.WriteLine(Schlez.Height);
                 scaleFactorX = (int)((picloc.Left + picloc.Right + Schlez.Source.Width) / 600);
                 scaleFactorY = (int)((picloc.Bottom + picloc.Top + Schlez.Source.Height) / 300);
            }));
            ...
            for (int i = 0; i < numOfHands; i++)
            {
                for (int j = 1; (j < 22 && !IsJointOverPic)  ; j++)
                {
                    ...
                    int x = (int)nodes[i][j].positionImage.x * scaleFactorX;
                    int y = (int)nodes[i][j].positionImage.y * scaleFactorY;

                    if (nodes[i][j].confidence <= 0) continue;
                    this.Dispatcher.Invoke(System.Windows.Threading.DispatcherPriority.Normal, new Action(delegate() { lblCor.Content = x + "," + y; }));
                    if (((x >= picloc.Right) && (x <= picloc.Right + Schlez.Source.Width)) && ((y >= picloc.Top) && (y <= picloc.Top + Schlez.Source.Height)) && (j == 1))
                    {
                       IsJointOverPic = true;
                    }
                }
        ...
        private void UpdateUI(Bitmap bitmap)
        {
            this.Dispatcher.Invoke(System.Windows.Threading.DispatcherPriority.Normal, new Action(delegate()
            {
                if (bitmap != null)
                {
                    ...
                    if (handWaving && isHandOnPic)
                    {
                        lblMessage.Content = "You King In The Pic!";
                        handTrigger = true;
                    }
